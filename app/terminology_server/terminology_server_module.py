...
class TerminologyServer:
    ...
    def do_expand(
        ...
        ecl=None,
        refset_id=None,
        value_set_server_id=None,
        sct_version=None,
        add_display_names=False,
    ):

        one_and_only_one_defined = (
            sum(
                [
                    int(bool(x not in ["", None]))
                    for x in [ecl, refset_id, value_set_server_id]
                ]
            )
            == 1
        )
        if not one_and_only_one_defined:
            print(
                "Require one and only one of ecl and value_set_server_id to be defined in expand_ecl"
            )
            print("ecl", ecl)
        ...
        if ecl:  # expand a piece of ecl as implicit ValueSet
            ...
            if sct_version:
                relative_url = (
                    "ValueSet/$expand?property=inactive&url=%s?fhir_vs=ecl/(%s)"
                    % (sct_version, ecl)
                )
            else:
                relative_url = (
                    "ValueSet/$expand?url=http://snomed.info/sct?fhir_vs=ecl/(%s)"
                    % (ecl)
                )
        elif refset_id:  # expand a SNOMED refset as implicit ValueSet
            if sct_version:
                relative_url = "ValueSet/$expand?url=%s?fhir_vs=refset/%s" % (
                    sct_version,
                    refset_id,
                )
            else:
                relative_url = (
                    "ValueSet/$expand?url=http://snomed.info/sct?fhir_vs=refset/%s"
                    % (refset_id)
                )
        else:  # expand an explicit ValueSet
            if sct_version:
                relative_url = (
                    "ValueSet/%s/$expand?system-version=http://snomed.info/sct%%7C%s"
                    % (value_set_server_id, sct_version)
                )
            else:
                ...
                for contained_item in extensional_valueset.expansion.contains:
                    if add_display_names:
                        ecl_response.append(
                            ...
                            % (
                                contained_item.code,
                                contained_item.display,
                                contained_item.inactive,
                            )
                        )
                    else:
                        ecl_response.append(
                            int(contained_item.code)
                        )  # trial addition12 jan22
            return ecl_response
        else:
