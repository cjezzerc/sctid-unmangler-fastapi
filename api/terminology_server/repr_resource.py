""" Utility function for representing a fhir resource in dot notation 
            ...
            if (thing_value is not None): 
                ...
                if (t is not str) and iterable(thing_value) and (not (type(thing_value)==list and type(thing_value[0])==str)):
                    if type(thing_value)==list:
                        decoration="[0..%s]" % (len(thing_value)-1)
                    else:
                        decoration="..."
                    repr_strings.append("%s %-50s %s:" % (indent_string, object_representation+"."+thing_name+decoration, padding_string))
