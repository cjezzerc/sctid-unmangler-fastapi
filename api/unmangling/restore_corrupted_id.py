    ...
    for analysis in analyses_list:
        if analysis.outcome_code == OutcomeCodes.POSSIBLE_CORRUPTION:
            ...
            if (analysis.r_cid is not None) ^ (analysis.r_did is not None):
                ...
                if analysis.sctid_provided in [analysis.r_cid, analysis.r_did]: #see comment below
                    analysis.outcome_code = OutcomeCodes.ANY_CORRUPTION_IS_SILENT

            elif (analysis.r_cid is not None) and (analysis.r_did is not None): #see comment below
                analysis.outcome_code = OutcomeCodes.POSSIBLE_CORRUPTION_AMBIG

                if analysis.sctid_provided in [analysis.r_cid, analysis.r_did]:
            ...
            # I think coded like this the intent is clear even if certain of the conditions can never be true