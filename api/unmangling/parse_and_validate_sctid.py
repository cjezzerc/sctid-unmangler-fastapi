    ...
    def parse_and_validate_sctid(self):
        ...
        while first_time_round_while:
            ...
            if type(self.sctid_string) != str:
                self.valid = False
                self.validation_message = (
                    "The supplied SCTID is of type %s but a string is required."
                    % type(self.sctid_string)
                )
                break
            #                                 #
            ###################################
            ...
            if (re.search(r"^[0-9]+$", self.sctid_string)) is None:
                self.valid = False
                self.validation_message = (
                    "A SCTID can only contain the digits 0-9 and no other characters (including spaces). The supplied SCTID was '%s'."
                    % self.sctid_string
                )
                break
            #                                      #
            ...
            if len_sctid < 6:
                self.valid = False
                self.validation_message = (
                    "The supplied SCTID is too short. It has a length of %s but the minimum required length is 6"
                    % len_sctid
                )
                break
            #                                #
            ...
            if len_sctid > 18:
                self.valid = False
                self.validation_message = (
                    "The supplied SCTID is too long. It has a length of %s but the maximum allowed length is 18"
                    % len_sctid
                )
                break
            #                                  #
            ...
            if not checkdigit.verhoeff_check(self.sctid_string):
                self.valid = False
                self.validation_message = (
                    "The supplied SCTID has the incorrect checkdigit"
                )
                break
            #                                #
            ##################################
            ...
            if self.partition_identifier not in ["00", "01", "02", "10", "11", "12"]:
                self.valid = False
                self.validation_message = (
                    "The supplied SCTID has a partition identifier of %s. The only allowed values are 00, 01, 02, 10, 11 or 12"
            ...
            if self.long_form_flag and len_sctid < 11:
                self.valid = False
                self.validation_message = (
                    "The supplied SCTID is a 'long form' type. It has a length of %s but the minimum required length for a long form type is 11"
                    % len_sctid
                )
                break
            #                                         #
        ...
        if self.namespace_identifier != 0:
            self.underscore_separated_form = (
                str(self.item_identifier)
                + "_"
                + str(self.namespace_identifier)
                + "_"
                + str(self.partition_identifier)
                + "_"
